...
xread(int fd, char *buffer, size_t len) {
  ...
  /* There is a 2GB limit when reading from a file, enforced by the INT_MAX
     value (2GB). If 'len' exceeds that limit, we read the file in 2GB chunks;
     otherwise perform the read in one single chunk. */
  unsigned int chunk_size = len > INT_MAX ? INT_MAX : len;

  while (len) {
  	n = read(fd, buffer + done, chunk_size);
	  if (n == 0) {
...
/* SN-Carlos: Windows specific header files */
#ifdef _WIN32
#include <windows.h>
#endif /* _WIN32 */
